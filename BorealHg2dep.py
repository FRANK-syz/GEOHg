...
# HgWetDep = gdal.Open("BorealHg results/2021Hg2dep.tif").ReadAsArray()
# HgWetDep = gdal.Open("BorealHg results/2023Hg2dep.tif").ReadAsArray()
HgWetDep = gdal.Open("BorealHg results/15-19Hg2dep.tif").ReadAsArray()

Fillgap=HgWetDep[:,0]/2+HgWetDep[:,-1]/2
HgWetDep = np.column_stack((HgWetDep, Fillgap))
...
mesh = ax.contourf(lon_grid, lat_grid, HgWetDep, 
                   levels=bounds, 
                   cmap=cmap, 
...
plt.title('Hg$^{II}$ deposition (μg m$^{-2}$ yr$^{-1}$)',fontsize=12, pad=20, y=-0.26, fontname='Arial')
# plt.savefig('BorealHg results/2021Hg2depfromMBOBB.png', transparent=True, dpi=300)
# plt.savefig('BorealHg results/2023Hg2depfromMBOBB.png', transparent=True, dpi=300)
plt.savefig('BorealHg results/15-19Hg2depfromMBOBB.png', transparent=True, dpi=300)
plt.show()
